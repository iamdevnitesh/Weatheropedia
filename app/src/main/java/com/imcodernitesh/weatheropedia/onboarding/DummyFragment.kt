...
class DummyFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_dummy, container, false).apply {
            ...
            findViewById<LottieAnimationView>(R.id.lottieAnimationView).setOnClickListener{
                // copy link to clipboard
                val clipboard = context?.getSystemService(android.content.Context.CLIPBOARD_SERVICE) as android.content.ClipboardManager
                val clip = android.content.ClipData.newPlainText("label", "https://github.com/iamdevnitesh/")
                clipboard.setPrimaryClip(clip)
                Toast.makeText(context, "Link copied to clipboard", Toast.LENGTH_SHORT).show()
            }

            findViewById<LottieAnimationView>(R.id.lottieAnimationView).setAnimation(
                param2 ?: R.raw.sattelite
            )
